...
int QUARTER_WIDTH = RESOLUTIONS[0][3] >>> 2;
int QUARTER_HEIGHT = RESOLUTIONS[1][3] >>> 2;

int BOX_SIZE = RESOLUTIONS[0][3]*50/600;
final float  DEGS_TO_RADS = (float)Math.PI/180f;
final String PATH_BASE = "C:\\Users\\richa\\OneDrive\\Documents\\processing-4.3.1-windows-x64\\processing-4.3.1\\sketch_3DTriTest\\";

Model[] testModels = new Model[15];
LinkedList<Model> testModelLinked;
Geometry[] testGeometry = new Geometry[9];//0 = cube, 1 = Tetrahedron, 2 = Pyramid, 3 = rhomohedron, 4 = rotating tri pair, 5 = giant tris in back, 6 = test billboard, 7 = cute flag
ModelColours[] testPallets = new ModelColours[11]; //0 = b&w cube, 1-8 above
Model[] smallerSet = new Model[9];
LinkedList<Model> linkedSmallerSet;

Triangle[] tris = new Triangle[10];
Dot[] testDot = new Dot[2];
LinkedList<Dot> testDotLinked;
LineObj[] testLines = new LineObj[4];
LinkedList<LineObj> testLineLinked;

Light[] light = new Light[2];
Light secondLight;

float[][] endPoints1 = {{0, 1.25f, -2.5f}, {0, -1.25f, 2.5f}, {2, -1.25f, 2.5f}};
float[][] endPoints2 = {{-5, 0, 2}, {5, 0, 2}};
//Cameras
...
int[] testBack = {0xFF00FFFF, 0xFF00FFFF, 0xFF00FFFF, 0xFF00FFFF, 0xFF00FFFF, 0xFF00FFFF, 0xFF00FFFF, 0xFF00FFFF, 0xFF00FFFF, 0xFF00FFFF,
...
int[][] backgrounds = {testBack, new int[RESOLUTIONS[0][3]*RESOLUTIONS[1][3]]};
int[] screen2 = new int[(RESOLUTIONS[0][3] >>> 2)*(RESOLUTIONS[1][3] >>> 2)];
PImage[] testImages = new PImage[2];
float sceneMag = 0;
float[] sceneCentre = {0, 0, 0};
byte[] testStencil = new byte[RESOLUTIONS[0][3]*RESOLUTIONS[1][3]];
PGraphics output;
BillboardImg[] sprites = new BillboardImg[2];
Billboard[] test = new Billboard[2];
LinkedList<Billboard> testBillboardLinked;
final int[] MAROON = {Colour.MAROON, Colour.RED, Colour.BLACK};
final int[] BLACK = {0x5500};
final int[] OPAQUE_BLACK = {Colour.BLACK};
int[][] endPoints = {{0, 1}};
int[][] endPointsSet2 = {{0, 1}, {1, 2}, {2, 0}};
int[][] endPointsSet3 = {{0, 1}, {1, 2}, {2, 3}, {3, 0}, {4, 5}, {5, 6}, {6, 7}, {7, 4}, {0, 4}, {1, 5}, {2, 6}, {3, 7}};
LinkedList<Light> lightsPrimary = new LinkedList<Light>();
LinkedList<Light> lightsSecondary = new LinkedList<Light>();
void setup(){
  ...
  light[0].setAmbientIntensity(0.03);
  ...
  light[1].setAmbientIntensity(0.03);
  ...
  testModels[7].setShininess(0.25);
  ...
  testModels[10].setShininess(0.001);
  ...
  testModels[12].setPosition(-0.35, -5.5, 8);
  ...
  testModels[14].setScale(2.5, 2.5, 2.5);
  ...
  sceneMag = VectorOperations.vectorMagnitude(tempCentre)*0.65;
...
void addToPosition(float[] position, float speed, float[] directionVector){
...
void draw(){
  ...
  ScreenMake.drawScene(output.pixels, eye, lightsPrimary, 1.5);
  ...
  ScreenMake.drawScene(screen2, eye2, lightsSecondary, 1.5);
  for(int i = miniXEnds[0]; i < miniXEnds[1]; i++){
    y = positionY;
    for(int j = miniYEnds[0]; j < miniYEnds[1]; j++){
